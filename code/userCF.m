% ======================== userCF.m =======================================
% This script runs the user based Collaborative filtering recommender sys
% using user similarities calculated on full rating matrix
...
% Calculating similarities between the users
SIM = zeros(m,m);
SIM(SIM == 0) = NaN;
...
% Running the loop for neighborhood size and finding errors on train & test
for L = 1:30
    tic;
    test_points = 0;
    train_points=0;
    ...
    for i = 1:m
        for j = 1:n
            ...
            if test(i,j)>0
                ...
                idx = find(train(:,j) > 0);
                [simVec, idxVec] = maxk(SIM(i,idx), L);
                
                closeMovie = idx(idxVec);
                num = simVec * (train(closeMovie, j));
                den = sum(abs(simVec));
            ...
            if train(i,j)>0
                ...
                closeMovie = idx(idxVec);
                num = simVec * (train(closeMovie, j)); % +u_avg(a, :))';
                den = sum(abs(simVec));
                ...
                train_ape = train_ape + abs(pred-train(i, j))/test(i, j);
